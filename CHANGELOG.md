...
That means the error summary in `<main>` is now generated by the `main` tag helper, so it honours `GovUkFrontendOptions.ErrorSummaryGeneration`: it's no longer rendered when `PrependToMainElement` isn't set, and it respects `DisableAutoFocus`.

`DateInputModelMetadata` is now public, so a custom `IDisplayMetadataProvider` or model binder can read the error message prefix and item types that a `DateInput` attribute contributed. It's in `ModelMetadata.AdditionalValues`, keyed on its own type, and `ModelMetadata.TryGetDateInputModelMetadata()` is now public too to get it out:

```csharp
if (modelMetadata.TryGetDateInputModelMetadata(out var dateInputModelMetadata))
{
    var prefix = dateInputModelMetadata.ErrorMessagePrefix;
}
```
