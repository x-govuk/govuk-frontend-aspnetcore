...
`DateInputModelMetadata` is now public, so a custom `IDisplayMetadataProvider` or model binder can read the error message prefix and item types that a `DateInput` attribute contributed. It's in `ModelMetadata.AdditionalValues`, keyed on its own type.
