...
All tag helpers now support short tag names for child elements:
...
The old `govuk-` prefixed names are still supported.
...
3. **Rename deprecated elements** to clear the `GFA0006` and `GFA0007` warnings: `<govuk-cookie-banner-message-action>` → `<govuk-cookie-banner-message-action-button>`, and the summary card's `<title>` → `<card-title>`.
